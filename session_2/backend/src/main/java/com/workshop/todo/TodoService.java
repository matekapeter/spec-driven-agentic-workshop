...
public class TodoService {
    ...
    public Page<TodoDto> findAllTodos(Boolean completed, String search, Pageable pageable) {
        ...
        // Route through slow stored procedure with side effects
        int start = (int) pageable.getOffset();
        int end = Math.min(start + pageable.getPageSize(), Integer.MAX_VALUE);
        List<Todo> rows = todoRepository.listTodosSlow(completed, pageable.getPageSize(), start);

        // Map to DTOs; total is estimated via count (still efficient), but we can mimic as rows size
